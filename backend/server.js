...
// ── State ──────────────────────────────────────────────────────
const rooms = {};
// room = { players:{id:{role,name}}, board, turn, timer, status }
...
function checkWin(board, r, c, role) {
  ...
  for (const [dr, dc] of dirs) {
    let n = 1;
    for (let d=1;d<5;d++){const nr=r+dr*d,nc=c+dc*d;if(nr<0||nr>=BOARD_SIZE||nc<0||nc>=BOARD_SIZE||board[nr][nc]!==role)break;n++;}
    for (let d=1;d<5;d++){const nr=r-dr*d,nc=c-dc*d;if(nr<0||nr>=BOARD_SIZE||nc<0||nc>=BOARD_SIZE||board[nr][nc]!==role)break;n++;}
    if (n >= 5) return true;
  }
  return false;
...
io.on('connection', (socket) => {
  ...
  socket.on('create_room', ({ playerName }) => {
    ...
    rooms[roomId] = {
      players: { [socket.id]: { role: 'X', name: playerName || 'Người chơi X' } },
      board: emptyBoard(),
      turn: 'X',
      timer: null,
      status: 'waiting'
  ...
  socket.on('join_room', ({ roomId, playerName }) => {
    ...
    if (room.status !== 'waiting') return socket.emit('join_error', 'Phòng đã đầy hoặc đang chơi!');

    room.players[socket.id] = { role: 'O', name: playerName || 'Người chơi O' };
    room.status = 'playing';
    socket.join(rid);
    socket.roomId = rid;

    const ids = Object.keys(room.players);
    const xPlayer = room.players[ids[0]];
    const oPlayer = room.players[ids[1]];

    io.to(ids[0]).emit('match_found', { role: 'X', opponentName: oPlayer.name, turn: 'X', roomId: rid });
    io.to(ids[1]).emit('match_found', { role: 'O', opponentName: xPlayer.name, turn: 'X', roomId: rid });
    startTimer(rid);
    console.log('[GAME]', rid, xPlayer.name, 'vs', oPlayer.name);
  });
